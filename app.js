...
Ext.extend(Ext.App, Ext.util.Observable, {
     ...
     */
    STATUS_EXCEPTION :          'Предупреждение',
    STATUS_VALIDATION_ERROR :   "Ошибка валидации",
    STATUS_ERROR:               "Ошибка",
    STATUS_NOTICE:              "Информация",
    STATUS_OK:                  "Ок",
    STATUS_HELP:                "Помощь",
    ...
    nav: function (){
        ...
        while( i-- ){
            var args = this.path.match(this._routes[i].pattern);
            if( args ){
                this.hideall();
                this.views[this._routes[i].callback].show().showid(args.slice(1));
            }
        }
    },
    path:''
...
Ext.onReady(function() {
    ...
    App.registerView("indexPage", indexPageSingleton());
    App.registerView("NewsList", NewsListSingleton());
    //App.views.News = NewsSingleton().hide();
    App.registerView("NewsAdd", NewsAddSingleton());
    //App.views.NewsAdd = new NewsAdd;
    //App.views.NewsEdit = NewsEditSingleton().hide();
    App.hideall();

    Ext.History.on('change', function() {
        if(Ext.History.getToken()!='null'){
            App.path = Ext.History.getToken();
        } else {
            App.path = location.pathname.substr(1);
        };
        App.nav();
    });
